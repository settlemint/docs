...
<Callout type="info">
  ...
  - [EVM Chains Guide: Setting up Graph Middleware](/building-with-settlemint/evm-chains-guide/setup-graph-middleware)
...
- `bunx settlemint scs subgraph codegen` - Generates the AssemblyScript types for your
  contracts ABI.
- `bunx settlemint scs subgraph build` - Compiles the WASM files based on the outputs
  generated by `bunx settlemint scs subgraph codegen`.
- `bunx settlemint scs subgraph deploy` - Deploys the WASM files to IPFS and updates the
