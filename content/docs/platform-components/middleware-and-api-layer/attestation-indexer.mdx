...
in the SettleMint IDE, via the **Terminal**, or using the **SettleMint CLI**.

<Tabs>
<Tab value="cli" label="SettleMint CLI">

#### Deploy using the SettleMint CLI

1. **Install the SettleMint SDK**:
   ```bash
   npm install -g @settlemint/sdk
   ```

2. **Deploy the attestation service**:
   ```bash
   # Deploy EAS contracts to your SettleMint application
   settlemint deploy attestation-service \
     --app-id YOUR_APP_ID \
...
3. **Get deployment information**:
   ...
   settlemint contracts list --app-id YOUR_APP_ID
   ```
...
### Example using the SettleMint SDK

```javascript
import { SettleMintSDK } from '@settlemint/sdk';
import { EAS } from '@settlemint/sdk-eas';
...
// Get EAS instance for your application
const eas = sdk.getEAS();

async function registerSchema() {
  try {
    // Define schema for social media ownership attestations
    const schema = {
      fields: [
      ...
      revocable: true,
      resolver: null // No custom resolver needed
    };
...
// Alternative: Using direct contract interaction
async function registerSchemaDirectly() {
  const { ethers } = require("ethers");
  
  // Get contract instances from SDK
  const contracts = await sdk.getContracts();
  const schemaRegistry = contracts.schemaRegistry;
  
  // Define schema string
  const schemaString = "string username, string platform, string handle";
  const resolverAddress = ethers.ZeroAddress;
  const revocable = true;
  
  // Register using contract directly
...
async function createAttestation(schemaUID) {
  try {
    ...
    // Create attestation using the SDK
    const attestationUID = await eas.createAttestation({
      schemaUID: schemaUID,
      data: attestationData,
      recipient: null, // Public attestation
...
// Alternative: Using the EAS SDK directly with SettleMint connection
async function createAttestationWithEASSdk() {
  const { EAS, SchemaEncoder } = require("@ethereum-attestation-service/eas-sdk");
  
  // Get connection details from SettleMint SDK
  const connection = await sdk.getConnection();
  const contracts = await sdk.getContracts();
  
  // Initialize EAS with SettleMint connection
  const easInstance = new EAS(contracts.eas.address);
  await easInstance.connect(connection.signer);
  ...
  const receipt = await tx.wait();
  console.log("✅ Attestation created! UID:", receipt.attestationUID);
  return receipt.attestationUID;
}

// CLI usage example
// settlemint eas create-attestation \
//   --app-id YOUR_APP_ID \
//   --schema-uid YOUR_SCHEMA_UID \
//   --data '{"username":"awesome_developer","platform":"GitHub","handle":"@devmaster"}'
...
async function verifyAttestation(attestationUID) {
  ...
  try {
    // Get attestation details using the SDK
    ...
    const decodedData = await eas.decodeAttestation(attestation);
    
    console.log('✅ Attestation Details:');
    console.log('Attester:', attestation.attester);
    console.log('Recipient:', attestation.recipient);
    console.log('Revoked:', attestation.revoked ? 'Yes' : 'No');
    console.log('Created:', new Date(attestation.time * 1000).toLocaleString());
...
// CLI usage example
// settlemint eas verify-attestation \
//   --app-id YOUR_APP_ID \
//   --attestation-uid YOUR_ATTESTATION_UID

// Alternative: Direct EAS SDK usage with SettleMint connection
async function verifyWithEASSdk(attestationUID) {
  const { EAS } = require("@ethereum-attestation-service/eas-sdk");
  
  // Get connection from SettleMint SDK
  const connection = await sdk.getConnection();
  const contracts = await sdk.getContracts();
  
  // Initialize EAS
  const easInstance = new EAS(contracts.eas.address);
  await easInstance.connect(connection.provider);
...
#### Example query implementation using the SettleMint SDK
...
// Get EAS instance with indexer
const eas = sdk.getEAS();

// Query attestations using the SDK
async function queryAttestations(schemaUID) {
  try {
    // Query using the SDK's built-in indexer integration
    const attestations = await eas.queryAttestations({
      where: {
        schemaId: schemaUID,
        revoked: false
      },
...
async function advancedQuery() {
  const attestations = await eas.queryAttestations({
    where: {
      AND: [
        { schemaId: 'YOUR_SCHEMA_UID' },
        { attester: '0x123...' },
        { time_gte: Math.floor(Date.now() / 1000) - 86400 } // Last 24 hours
      ]
    },
    orderBy: 'time_DESC',
    limit: 20
...
// Direct GraphQL query using SDK connection
async function queryViaGraphQL(schemaId) {
  const indexer = await sdk.getIndexer('attestation');
...
// CLI usage example
// settlemint eas query-attestations \
//   --app-id YOUR_APP_ID \
//   --schema-uid YOUR_SCHEMA_UID \
//   --limit 10
```
