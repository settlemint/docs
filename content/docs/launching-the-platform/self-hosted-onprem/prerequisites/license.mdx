...
<Callout type="warning">
  ...
  secret manager (for example, 1Password with `op://` references) and never
...
Add the following block to your `values.yaml` (use `op://` or your secret
manager references instead of plaintext):
