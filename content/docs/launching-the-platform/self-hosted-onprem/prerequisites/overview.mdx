...
<Tabs items={['Infrastructure', 'Databases & Cache', 'Security & Storage']}>
  ...
  <Tab value="Security & Storage">
    ...
    ### Email (SMTP) for Passwordless Login
    - Required to send sign-in links or codes
    - Provide host, port, user, password, and from address
    - Configure under `auth.providers.passwordless` in your Helm values
...
- [ ] SettleMint license and registry credentials
...
- [ ] SMTP server details (if using passwordless email login)
- [ ] Metrics endpoints
