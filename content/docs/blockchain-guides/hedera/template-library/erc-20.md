...
blockchain networks such as Hedera, that have value and can be sent and
...
_Note:_ On the Hedera Mainnet time to time you can get timeouts
errors(`Service Unavailable` in the IDE UI). JSON RPC relay in Hedera is a
complex software which relies on multiple components (the consensus nodes, and
the mirror node). One Ethereum TX can generate more than ten Hedera txs and if
one of the tx fails due to the connection timeout, it can cause the problem.
However, it's likely to have the problem with the smart contract deployment
only. If you can keep trying a couple of times and then once your contract is
deployed successfully. The subsequent contract calls should not have this kind
of problem.
