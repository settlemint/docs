...
ERC-721 tokens are blockchain-based assets, issued on all EVM compatible blockchain networks such as Ethereum, that have value and can be sent and received. Contrary to the ERC-20 tokens, these ERC-721 tokens are non-fungible, meaning that two tokens from the same smart contract are not equivalent.
