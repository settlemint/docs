...
>
  ...
  <div style={{ fontSize: "16px", fontWeight: "normal", marginTop: "5px" }}>
    ...
    TypeScript. Transaction functions are added to handle common
    operations—creating new entries, reading existing ones, updating data, and
    deleting records. These functions follow familiar CRUD patterns but are
    designed to work within the immutable environment of a blockchain. Events
    are emitted during each transaction to make state changes observable by
    off-chain systems or external services.
    <br> </br>
