...
<Tabs items={['Platform UI', 'SDK CLI', 'SDK JS']}>
  <Tab value="Platform UI">
    ...
    6. Click on 'Confirm' and wait for the Custom Deployment to be in the Running status. [View the list of statuses](/reference/statuses).
  </Tab>
...
2. **Read-Only Filesystem**: The filesystem is read-only. For data persistence, consider using:
   ...
   - Hasura: A GraphQL engine that provides a scalable database solution. See [Hasura](/building-with-settlemint/hasura-backend-as-a-service).
