...
dedicated cluster, select a cloud provider and data center, and pick a resource
pack (small, medium, or large) that can be scaled later.

Once your node is deployed, SettleMint provides access to the Insights tab for
monitoring tools. Since Hedera is a public network, you can use Hedera’s public
blockchain explorer to track transactions and network activity.
