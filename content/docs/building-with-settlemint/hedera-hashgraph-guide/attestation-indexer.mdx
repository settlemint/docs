...
2. **Deploy the attestation service to Hedera**:
   ```bash
   # Deploy EAS contracts to your SettleMint application on Hedera
...
### Example using the SettleMint SDK for Hedera

```javascript
import { SettleMintSDK } from '@settlemint/sdk';
import { EAS } from '@settlemint/sdk-eas';

// Initialize the SDK for Hedera network
...
// Get EAS instance for your Hedera application
...
async function registerSchema() {
  try {
    ...
    console.log('🚀 Registering schema for social media ownership on Hedera...');
    
    // Register the schema using the SDK
    const schemaUID = await eas.registerSchema(schema);
    
    console.log('✅ Schema registered successfully on Hedera! UID:', schemaUID);
    return schemaUID;
  } catch (error) {
...
// CLI usage for Hedera
// settlemint eas register-schema \
//   --app-id YOUR_APP_ID \
//   --network hedera-testnet \
//   --schema "string username, string platform, string handle" \
//   --revocable true
...
async function createAttestation(schemaUID) {
  try {
    ...
    // Create attestation using the SDK on Hedera
    ...
    console.log('✅ Attestation created successfully on Hedera! UID:', attestationUID);
...
async function verifyAttestation(attestationUID) {
  console.log('🔍 Verifying attestation on Hedera:', attestationUID);
  ...
  try {
    ...
    console.log('✅ Attestation details on Hedera:');
...
#### Example query implementation using the SettleMint SDK for Hedera
...
// Query attestations using the SDK on Hedera
async function queryAttestations(schemaUID) {
  try {
    ...
    console.log(`Found ${attestations.length} attestations on Hedera`);
...
console.log("Attestations on Hedera:", attestations);
```
