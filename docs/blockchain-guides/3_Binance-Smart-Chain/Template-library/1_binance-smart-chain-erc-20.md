...
ERC-20 tokens are blockchain-based assets, issued on the Binance Smart Chain network, that have value and can be sent and received. These tokens are fungible, meaning they can be exchanged with another token of the same type because they have identical properties and there is an equal value. For example, the BEP-20 token of Alice is exactly the same as the BEP-20 token of Bob. They can exchange their token without consequences.

Examples of fungible assets are currencies, stocks of a company, bonds, gold and other precious metals.

## BEP-20 smart contract features

A BEP-20 smart contract is used to create fungible tokens and bring them to the blockchain. This process is called minting. It also keeps track of the total supply as well as the balances of users as they exchange their tokens.

The BEP-20 smart contract on the SettleMint platform has the following features:
...
By default, the account that deploys the BEP-20 smart contract gets 1,000,000 tokens. You can change this behaviour by modifying the **“constructor”** in **“GenericToken.sol”**. If you do not mint tokens in the constructor, make sure to mint some after the deployment.
...
## Deploying a BEP-20 smart contract
...
As soon as you are happy with the changes you made, just click on **“deploy”** in the **“task runner”** of the IDE and after a few seconds, your BEP-20 smart contract should be deployed on the network of your choice.

The **“GenericToken.ts”** script in the **“test”** folder showcases all the functionalities of the BEP-20 standard. It shows you how to use the smart contract in your dapp.

## BEP-20 with meta transactions
...
When looking at the GenericTokenMeta.sol smart contract in the IDE we can see the main differences with the basic BEP20 being the following:
...
### Sending a meta transaction to the BEP-20 contract

Sending a meta transaction is slightly different than sending a regular transaction, but the template set comes with an example in which 10 tokens are transferred between two wallets without BNB.
...
In that expression, `transfer` is the BEP-20 function we want to execute, `walletTwoAddress` is the account that will receive the tokens and the last parameter is the amount of tokens to be transferred.
...
This is done in the templateset by deploying an BEP-20 contract.
...
The BEP-20 crowdsale templateset we provide is designed to give you full flexibility to modify these different parameters according to your requirements.
...
The first method is where the whitelisted buyers can send BNB to the contract directly. The equivalent amount of tokens will be calculated automatically. The calculation is done by first converting the BNB to USD leveraging Chainlink Oracle. Then, the USD is converted to the equivalent amount of tokens using the `_usdRate` set in the crowdsale.
...
- Who do not know how to send BNB to a contract
- Support payments in other forms other than BNB. For example, fiat payments can be supported. The buyer would send fiat to the crowdsale admin. The crowdsale admin would then call this function to allocate the equivalent amount of tokens to the buyer.
...
1. An BEP20 token, the token to be sold
